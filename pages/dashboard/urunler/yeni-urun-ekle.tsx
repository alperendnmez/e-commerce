...
export default function NewProductPage() {
  ...
  const applyBulkPrice = () => {
    ...
    if (bulkPriceInput && combinations.length > 0) {
      const price = parseFloat(bulkPriceInput.value)
      
      if (!isNaN(price) && price >= 0) {
        // Her kombinasyon için fiyat değerini güncelle
        const updatedVariantsData = { ...variantsData }
        
        combinations.forEach(combo => {
          const comboKey = combo.join('-')
          
          // O kombinasyonun mevcut verisi yoksa oluştur
          if (!updatedVariantsData[comboKey]) {
            updatedVariantsData[comboKey] = {
              price: price,
              comparativePrice: 0,
              costPerItem: 0,
              stock: 0,
            ...
            // Varsa sadece fiyat değerini güncelle
            updatedVariantsData[comboKey].price = price;
          }
        })
        ...
        toast({
          title: "Başarılı",
          description: `Tüm varyantlar için fiyat ${price.toFixed(2)}₺ olarak güncellendi.`,
        })
      } else {
      toast({
        title: "Hata",
  ...
  const applyBulkStock = () => {
    ...
    if (bulkStockInput && combinations.length > 0) {
      const stock = parseInt(bulkStockInput.value)
      
      if (!isNaN(stock) && stock >= 0) {
        // Her kombinasyon için stok değerini güncelle
        const updatedVariantsData = { ...variantsData }
    ...
    combinations.forEach(combo => {
          ...
          if (!updatedVariantsData[comboKey]) {
            ...
            // Varsa sadece stok değerini güncelle
            updatedVariantsData[comboKey].stock = stock
          }
        })
    ...
    toast({
      title: "Başarılı",
          description: `Tüm varyantlar için stok ${stock} olarak güncellendi.`,
        })
      } else {
        toast({
          title: "Hata",
          description: "Lütfen geçerli bir stok miktarı girin",
          variant: "destructive"
        })
      }
    }
  ...
  // Varyant veri güncelleme işlevi
  const handleVariantDataChange = (comboKey: string, field: 'price' | 'stock' | 'comparativePrice' | 'costPerItem' | 'sku' | 'barcode', value: number | string) => {
    setVariantsData(prev => {
      // Daha önce bu kombinasyon için bir veri yoksa, varsayılan değerlerle oluştur
      if (!prev[comboKey]) {
        return {
          ...prev,
          [comboKey]: {
            price: field === 'price' ? (typeof value === 'number' ? value : 0) : 0,
            comparativePrice: field === 'comparativePrice' ? (typeof value === 'number' ? value : 0) : 0,
            costPerItem: field === 'costPerItem' ? (typeof value === 'number' ? value : 0) : 0,
            stock: field === 'stock' ? (typeof value === 'number' ? value : 0) : 0,
            images: [],
            sku: field === 'sku' ? (typeof value === 'string' ? value : '') : '',
            barcode: field === 'barcode' ? (typeof value === 'string' ? value : '') : ''
          }
        };
      }
  ...
  // Toplu karşılaştırma fiyatı uygulaması için yeni fonksiyon ekleyelim
  const applyBulkComparativePrice = () => {
    ...
    if (bulkComparativePriceInput && combinations.length > 0) {
      const bulkComparativePrice = parseFloat(bulkComparativePriceInput.value)
      
      if (!isNaN(bulkComparativePrice)) {
        // Her kombinasyon için karşılaştırma fiyatını güncelle
        const updatedVariantsData = { ...variantsData }
        ...
        combinations.forEach(combo => {
          ...
          if (!updatedVariantsData[comboKey]) {
            updatedVariantsData[comboKey] = {
              price: 0,
              comparativePrice: bulkComparativePrice,
              costPerItem: 0,
              stock: 0,
              images: [],
              sku: '',
            ...
            // Varsa güncelle
            updatedVariantsData[comboKey].comparativePrice = bulkComparativePrice
          }
        })
        ...
        toast({
          title: "Başarılı",
          description: `Tüm varyantlar için karşılaştırma fiyatı ${bulkComparativePrice}₺ olarak güncellendi.`,
        })
      }
    }
  }

  // Toplu maliyet uygulaması
  const applyBulkCost = () => {
    const bulkCostInput = document.getElementById('bulkCost') as HTMLInputElement
    
    if (bulkCostInput && combinations.length > 0) {
      const costPerItem = parseFloat(bulkCostInput.value)
      
      if (!isNaN(costPerItem) && costPerItem >= 0) {
        // Her kombinasyon için maliyet değerini güncelle
        const updatedVariantsData = { ...variantsData }
        ...
        combinations.forEach(combo => {
          ...
          if (!updatedVariantsData[comboKey]) {
            updatedVariantsData[comboKey] = {
              price: updatedVariantsData[comboKey]?.price || 0,
              comparativePrice: updatedVariantsData[comboKey]?.comparativePrice || 0,
              costPerItem: costPerItem,
              stock: updatedVariantsData[comboKey]?.stock || 0,
              images: [],
              sku: updatedVariantsData[comboKey]?.sku || '',
              barcode: updatedVariantsData[comboKey]?.barcode || ''
            }
          } else {
            // Varsa sadece maliyet değerini güncelle
            updatedVariantsData[comboKey].costPerItem = costPerItem
          }
        })
        ...
        toast({
          title: "Başarılı",
          description: `Tüm varyantlar için maliyet ${costPerItem.toFixed(2)}₺ olarak güncellendi.`,
        })
      } else {
      toast({
        title: "Hata",
          description: "Lütfen geçerli bir maliyet girin",
          variant: "destructive"
        })
      }
    }
  ...
  // SKU öneki uygulama işlevi
  const applyBulkSkuPrefix = () => {
    const bulkSkuPrefixInput = document.getElementById('bulkSkuPrefix') as HTMLInputElement
    
    if (bulkSkuPrefixInput && combinations.length > 0) {
      const skuPrefix = bulkSkuPrefixInput.value.trim()
      
      if (skuPrefix) {
        // Her kombinasyon için SKU değerini güncelle
        const updatedVariantsData = { ...variantsData }
        
        combinations.forEach((combo, index) => {
          const comboKey = combo.join('-')
          const newSku = `${skuPrefix}-${index + 1}` // SKU öneki + varyant sıra numarası
          ...
          if (!updatedVariantsData[comboKey]) {
            updatedVariantsData[comboKey] = {
              price: updatedVariantsData[comboKey]?.price || 0,
              comparativePrice: updatedVariantsData[comboKey]?.comparativePrice || 0,
              costPerItem: updatedVariantsData[comboKey]?.costPerItem || 0,
              stock: updatedVariantsData[comboKey]?.stock || 0,
              images: [],
              sku: newSku,
              barcode: updatedVariantsData[comboKey]?.barcode || ''
            ...
            // Varsa sadece SKU değerini güncelle
            updatedVariantsData[comboKey].sku = newSku
          }
        })
        ...
        toast({
          title: "Başarılı",
          description: `Tüm varyantlar için SKU öneki "${skuPrefix}" olarak uygulandı.`,
        })
      } else {
        toast({
          title: "Uyarı",
          description: "Lütfen geçerli bir SKU öneki girin",
          variant: "destructive"
        })
      }
    }
  ...
  // Barkod öneki uygulama işlevi
  const applyBulkBarcodePrefix = () => {
    const bulkBarcodePrefixInput = document.getElementById('bulkBarcodePrefix') as HTMLInputElement
    
    if (bulkBarcodePrefixInput && combinations.length > 0) {
      const barcodePrefix = bulkBarcodePrefixInput.value.trim()
      
      if (barcodePrefix) {
        // Her kombinasyon için barkod değerini güncelle
        const updatedVariantsData = { ...variantsData }
        
        combinations.forEach((combo, index) => {
          const comboKey = combo.join('-')
          // Barkod için sıra numarasını 3 haneli olarak formatla (001, 002, ...)
          const sequenceNumber = String(index + 1).padStart(3, '0')
          const newBarcode = `${barcodePrefix}${sequenceNumber}`
          ...
          if (!updatedVariantsData[comboKey]) {
            ...
            // Varsa sadece barkod değerini güncelle
            updatedVariantsData[comboKey].barcode = newBarcode
          }
        })
        ...
        toast({
          title: "Başarılı",
          description: `Tüm varyantlar için barkod öneki "${barcodePrefix}" olarak uygulandı.`,
        })
      } else {
        toast({
          title: "Uyarı",
          description: "Lütfen geçerli bir barkod öneki girin",
          variant: "destructive"
        })
      }
    }
  ...
  // Varyantsız ürün görseli yükleme işlevi
        ...
        <div className="flex items-start">
          ...
          <div>
            ...
            <ul className="mt-1 text-sm list-disc list-inside ml-1">
              ...
              <li>En az bir varyant grubu (renk, beden vb.) eklemelisiniz.</li>
              <li>Her varyant kombinasyonu için fiyat ve stok bilgisi girmelisiniz.</li>
          ...
          <div className="w-full mb-8">
            <div className="flex justify-between mb-2">
              <div className={`flex flex-col items-center ${step >= 1 ? 'text-primary' : 'text-muted-foreground'}`}>
                <div className={`w-10 h-10 rounded-full flex items-center justify-center mb-2 ${step >= 1 ? 'bg-primary text-primary-foreground' : 'bg-muted'}`}>1</div>
                <span className="text-sm">Ürün Bilgileri</span>
              </div>
              <div className="grow mx-2 flex items-center">
                <div className={`h-1 w-full ${step >= 2 ? 'bg-primary' : 'bg-muted'}`}></div>
              </div>
              <div className={`flex flex-col items-center ${step >= 2 ? 'text-primary' : 'text-muted-foreground'}`}>
                <div className={`w-10 h-10 rounded-full flex items-center justify-center mb-2 ${step >= 2 ? 'bg-primary text-primary-foreground' : 'bg-muted'}`}>2</div>
                <span className="text-sm">Varyantlar</span>
              </div>
              <div className="grow mx-2 flex items-center">
                <div className={`h-1 w-full ${step >= 3 ? 'bg-primary' : 'bg-muted'}`}></div>
              </div>
              <div className={`flex flex-col items-center ${step >= 3 ? 'text-primary' : 'text-muted-foreground'}`}>
                <div className={`w-10 h-10 rounded-full flex items-center justify-center mb-2 ${step >= 3 ? 'bg-primary text-primary-foreground' : 'bg-muted'}`}>3</div>
                <span className="text-sm">Kombinasyonlar</span>
              </div>
            </div>
          </div>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      {variantGroups.length === 0 && (
                        <>
                        <div className="space-y-2">
                            <Label htmlFor="basePrice">Ürün Fiyatı (₺) *</Label>
                          <Input
                            id="basePrice"
                            type="number"
                      ...
                      <div className="space-y-2">
                        <Label htmlFor="comparativePrice">Karşılaştırma Fiyatı (₺)</Label>
                        <Input
                          id="comparativePrice"
                          type="number"
                      ...
                      <div className="space-y-2">
                        <Label htmlFor="costPerItem">Ürün Başına Maliyet (₺)</Label>
                        <Input
                          id="costPerItem"
                          type="number"
                      ...
                      <div className="space-y-2">
                        <div className="flex items-center justify-between">
                          <Label htmlFor="taxIncluded">Bu üründen vergi al</Label>
                          <Switch
                            id="taxIncluded"
                      ...
                      {!productData.taxIncluded && productData.costPerItem && productData.basePrice && variantGroups.length === 0 && (
                        <div className="mt-3 p-3 border rounded-md bg-muted/20">
                          <h3 className="text-sm font-medium mb-2">Kar Hesabı</h3>
                      ...
                      <div className="space-y-2">
                        <Label htmlFor="categoryId" className="text-base font-medium">Kategori</Label>
                        <Select
                      ...
                      <div className="space-y-2">
                        <Label htmlFor="brandId" className="text-base font-medium">Marka</Label>
                        <Select
                      ...
                      {/* Kargo Bilgileri */}
                      <div className="space-y-4 col-span-2 mt-6 border-t pt-6">
                        <h2 className="text-lg font-medium">Kargo Bilgileri</h2>
                        
                        <div className="space-y-2">
                          <div className="flex items-center justify-between">
                            <Label htmlFor="isPhysicalProduct">Bu bir fiziksel üründür</Label>
                            <Switch
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                            <div className="md:col-span-2 grid grid-cols-3 gap-4">
                              ...
                              <div className="col-span-1 space-y-2">
                                <Label htmlFor="weightUnit">Birim</Label>
                                <Select
                            ...
                            <div className="space-y-2">
                              <Label htmlFor="countryOfOrigin">Menşe Ülke/Bölge</Label>
                              <Select
                            ...
                            <div className="space-y-2">
                              <Label htmlFor="hsCode">Armonize Sistem (HS) Kodu</Label>
                              <Input
                                id="hsCode"
                    ...
                    <div className="space-y-2">
                      <Label htmlFor="description" className="text-base font-medium">Ürün Açıklaması</Label>
                      <Textarea
                        id="description"
                    ...
                    <div className="space-y-4">
                      <h3 className="text-base font-medium">SEO Bilgileri</h3>
                      <div className="space-y-2">
            ...
            {step === 2 && (
              <>
                <CardHeader>
                  ...
                  <CardDescription>Ürün varyantlarını oluşturun (renk, beden, malzeme vb.)</CardDescription>
                </CardHeader>
                <CardContent className="space-y-6">
                  <div className="bg-muted/50 p-4 rounded-lg border border-muted">
                    <h3 className="text-sm font-medium mb-2">Varyant Nedir?</h3>
                    <p className="text-sm text-muted-foreground">
                      Varyantlar, aynı ürünün farklı seçeneklerini tanımlar. Örneğin, bir tişört için renk (siyah, beyaz, kırmızı) ve beden (S, M, L, XL) varyantları oluşturabilirsiniz.
                    </p>
                  </div>
                  ...
                  <div className="space-y-4">
                    <div className="flex items-end gap-4">
                      <div className="flex-1 space-y-2">
                        <Label htmlFor="groupName" className="text-base font-medium">Varyant Grubu Adı</Label>
                        <Input
                          id="groupName"
                    ...
                    {variantGroups.length > 0 && (
                      <div className="mt-6">
                        <h3 className="text-base font-medium mb-3">Eklenen Varyant Grupları</h3>
                        <div className="space-y-4">
                              ...
                              <CardHeader className="py-3 px-4 bg-muted/30">
                                <div className="flex items-center justify-between">
                                  ...
                                  <div className="flex items-center gap-2">
                                    <Badge variant="outline">{group.values.length} değer</Badge>
                                    <Button 
                                      variant="ghost" 
                                      size="icon" 
                                      ...
                                      onClick={() => {
                                        ...
                                        toast({
                                          title: "Bilgi",
                                          description: `"${group.name}" varyant grubu silindi.`,
                                        });
                                      }}
                                    >
                                ...
                                <div className="space-y-3">
                                  <div className="flex items-end gap-2">
                                    <div className="flex-1">
                                      <Input
                                        placeholder={`${group.name} değeri ekle (Örn: ${group.name === 'Renk' ? 'Siyah, Beyaz' : group.name === 'Beden' ? 'S, M, L' : 'Değer'}`}
                                        value={valueInput}
                                  ...
                                  {group.values.length > 0 ? (
                                    <div className="flex flex-wrap gap-2 mt-3">
                                      {group.values.map((val, j) => (
                                        <Badge key={j} variant="secondary" className="px-3 py-1 text-sm">
                                          {val}
                                          <button 
                    ...
                    {variantGroups.length === 0 && (
                      <div className="flex items-center justify-center h-40 border-2 border-dashed rounded-lg">
                        <div className="text-center">
                          <p className="text-muted-foreground">Henüz varyant grubu eklenmedi</p>
                          <p className="text-xs text-muted-foreground mt-1">Örn: Renk, Beden, Malzeme</p>
                        </div>
                      </div>
                    )}
                  </div>
                  ...
                  {combinations.length === 0 && (
                    <div className="flex items-center justify-center h-40 border-2 border-dashed rounded-lg">
                      <div className="text-center">
                        {variantGroups.length === 0 ? (
                          <>
                            <p className="text-muted-foreground">Bu ürün varyantları olmadan kaydedilecek.</p>
                            <p className="text-muted-foreground mt-2">Ürün fiyatı: <span className="font-medium">{parseFloat(String(productData.basePrice)).toLocaleString('tr-TR', {style: 'currency', currency: 'TRY'})}</span></p>
                            <Button 
                              variant="outline" 
                              className="mt-4"
                              onClick={() => setStep(1)}
                          ...
                          <>
                            <p className="text-muted-foreground">Varyant grupları eklenmedi veya değer girilmedi.</p>
                            <Button 
                              variant="outline" 
                              className="mt-4"
                              onClick={() => setStep(2)}
                  ...
                  {combinations.length > 0 && (
                    <div className="space-y-6">
                      ...
                      <div className="space-y-4 p-4 border rounded-md">
                        <h3 className="font-semibold text-base">Toplu İşlemler</h3>
                        
                        <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
                          {/* Toplu Fiyat */}
                          <div className="space-y-2">
                            <Label htmlFor="bulkPrice">Tüm Kombinasyonlar İçin Fiyat</Label>
                          ...
                          <div className="space-y-2">
                            <Label htmlFor="bulkComparativePrice">Tüm Kombinasyonlar İçin Karşılaştırma Fiyatı</Label>
                            <div className="flex space-x-2">
                              <Input
                              ...
                              />
                              <Button onClick={applyBulkComparativePrice} size="sm">
                                Uygula
                              </Button>
                            </div>
                          </div>
                          ...
                          {/* Toplu Stok */}
                          <div className="space-y-2">
                            <Label htmlFor="bulkStock">Tüm Kombinasyonlar İçin Stok</Label>
                            <div className="flex space-x-2">
                              <Input
                                id="bulkStock"
                                type="number"
                                ...
                                placeholder="Stok miktarı"
                              />
                              <Button onClick={applyBulkStock} size="sm">
                                Uygula
                              </Button>
                            </div>
                          </div>
                          ...
                          {/* Toplu Maliyet */}
                          <div className="space-y-2">
                            <Label htmlFor="bulkCost">Tüm Kombinasyonlar İçin Maliyet</Label>
                            <div className="flex space-x-2">
                              <Input
                                id="bulkCost"
                                type="number"
                                ...
                                placeholder="Maliyet"
                              />
                              <Button onClick={applyBulkCost} size="sm">
                                Uygula
                              </Button>
                        </div>
                      </div>
                          ...
                          {/* Toplu SKU Öneki */}
                          <div className="space-y-2">
                            <Label htmlFor="bulkSkuPrefix">SKU Öneki + Varyant ID</Label>
                            <div className="flex space-x-2">
                              <Input
                                id="bulkSkuPrefix"
                                type="text"
                              ...
                              />
                              <Button onClick={applyBulkSkuPrefix} size="sm">
                                Uygula
                              </Button>
                            </div>
                          ...
                          {/* Toplu Barkod Öneki */}
                          <div className="space-y-2">
                            <Label htmlFor="bulkBarcodePrefix">Barkod Öneki + Varyant ID</Label>
                            <div className="flex space-x-2">
                              <Input
                              ...
                              />
                              <Button onClick={applyBulkBarcodePrefix} size="sm">
                                Uygula
                              </Button>
                            </div>
                          ...
                          <div className="space-y-2 col-span-full">
                            <div className="flex items-center justify-between">
                              <Label htmlFor="taxIncludedVariants">Bu üründen vergi al</Label>
                              <Switch
                      ...
                      {/* Varyant kombinasyonları tablosu */}
                      <div className="border rounded-md">
                        <table className="w-full text-sm">
                          <thead>
                            <tr className="bg-muted/50 border-b">
                              <th className="py-2 px-4 text-left font-medium">Kombinasyon</th>
                              <th className="py-2 px-4 text-right font-medium">Fiyat (₺)</th>
                              <th className="py-2 px-4 text-right font-medium">Karşılaştırma Fiyatı (₺)</th>
                              <th className="py-2 px-4 text-right font-medium">Maliyet (₺)</th>
                              <th className="py-2 px-4 text-right font-medium">Stok</th>
                              <th className="py-2 px-4 text-right font-medium">SKU</th>
                              <th className="py-2 px-4 text-right font-medium">Barkod</th>
                              <th className="py-2 px-4 text-right font-medium">Görseller</th>
                              {!productData.taxIncluded && <th className="py-2 px-4 text-right font-medium">Kar / Marj</th>}
                            </tr>
                          </thead>
                          <tbody>
